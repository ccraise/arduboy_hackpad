...
// 设置一个范围，鼠标可以在这个范围内随机移动  
#define MOUSE_MOVE_RANGE 200  
// LED连接的引脚  
#define PIN_LED 13 // 假设LED连接到数字引脚13
...
const char ms_act_title_11[] PROGMEM = "Auto move";
...
void show_action_title() {
  ...
  if (get_menu_value() == 0) { //键盘
    ...
    if (get_action_value() == 12) arduboy.print(read_flash_str(act_title_exit));
  } else if (get_menu_value() == 2) { //password
...
void loop() {
  ...
  if (get_pos() == 0) {  // 登录屏幕
    login_screen();
  } else if (get_pos() == 1) {  // 主屏，功能选择界面
    main_screen();
  } else {  // 功能操作界面
    do_func();
  }
  hold_screen_by_btn(); // 如果按键没有抬起则暂停程序
}
...
// 平滑移动鼠标的函数  
void smoothMove(int startX, int startY, int endX, int endY, unsigned long duration) {  
    unsigned long startTime = millis();  
    int steps = 20; // 可以根据需要调整步数来影响平滑度  
    ...
    for (int i = 0; i < steps; i++) {  
      ...
      // 将鼠标移动到当前位置  
      Mouse.move(currentX, currentY, 0);  
      ...
      unsigned long sleepTime = (duration / steps) - (elapsedTime % (duration / steps));  
    ...
    // 移动到最终位置（确保精确性）  
    Mouse.move(endX, endY, 0);  
}  
   
// 随机移动鼠标
void ms_move_rnd(){
  if(key_cache == "waiting" || get_btn_pressed() == 1){
    return;
  }
      // 随机生成目标x和y坐标  
    int targetX = randomInRange(-MOUSE_MOVE_RANGE + currentX, MOUSE_MOVE_RANGE + currentX);  
    int targetY = randomInRange(-MOUSE_MOVE_RANGE + currentY, MOUSE_MOVE_RANGE + currentY);  
  
    // 平滑地移动鼠标到目标位置  
    smoothMove(currentX, currentY, targetX, targetY, randomInRange(2000, 15000));  
...
void screen_mouse() {
  ...
  show_menu_title();
  arduboy.print(read_flash_str(char_n));
  if (get_action_value() == 11) arduboy.print(key_cache);
}
//显示输出
void screen_show() {
